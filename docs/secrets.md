...
When a secret is exposed outside its intended boundary, rotate it at the provider first, update the secret store, deploy, and verify the old credential no longer works. Treat local `server/.env` as sensitive. Frontend env files, if they exist in another repo, must be handled with the same care.
